...
#define NS_SSE_P       (16 / sizeof(NS_SSE_INT))
#define NS_SSE_NEG_INF ((NS_SSE_INT)(1<<(8*sizeof(NS_SSE_INT)-1)))

static uint8_t *ns_prep(void *km, const char *ns, int32_t nl, const char *as, int32_t al, const ns_opt_t *opt, uint8_t **aas_, int8_t **donor_, int8_t **acceptor_)
{
	int32_t i, j, l;
...
void ns_splice_i16(void *km, const char *ns, int32_t nl, const char *as, int32_t al, const ns_opt_t *opt, ns_rst_t *r)
{
	...
	r->n_cigar = 0;
	nas = ns_prep(km, ns, nl, as, al, opt, &aas, &donor, &acceptor);
	ns_gen_prof(int16_t, km, aas, al, opt, NS_SSE_NEG_INF, &mem_ap, &ap);
	...
	H3[-1] = _mm_insert_epi16(H3[-1], 0, 0);
	H2[-1] = _mm_insert_epi16(H2[-1], -opt->fs, 0);
	H1[-1] = _mm_insert_epi16(H1[-1], -opt->fs, 0);
	...
	for (i = 2; i < nl; ++i) {
		...
		if (i > 2) { // FIXME: this is close but not correct
			H3[-1] = _mm_insert_epi16(_mm_slli_si128(H3[slen - 1], 2), NS_SSE_NEG_INF, 0);
			H2[-1] = _mm_insert_epi16(_mm_slli_si128(H2[slen - 1], 2), NS_SSE_NEG_INF, 0);
			H1[-1] = _mm_insert_epi16(_mm_slli_si128(H1[slen - 1], 2), NS_SSE_NEG_INF, 0);
		}
		for (j = 0; j < slen; ++j) {
			__m128i h, t, u, v;
